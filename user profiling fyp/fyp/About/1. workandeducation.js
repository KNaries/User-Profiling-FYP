...
function getWork(User) {
  var variables = {
    ...
    "collectionToken": "YXBwX2NvbGxlY3Rpb246MT AwMDAyOTA3NTc1NjQyOjIzMjcxNTgyMjc6MjAy",
    "pageID": `${User}`,
    ...
    "scale": 1,
    "sectionToken": "YXBwX3NlY3Rpb246MTAwMDAyOTA3NTc1NjQyOjIzMjcxNTgyMjc=",
    "showReactions": true,
    "userID": `${User}`,
  ...
  params += '__a=1&';
  params += '__req=l&';
  params += "__hs=19978.HYP%3Acomet_pkg.2.1..2.1&";
  params += "dpr=1&";
  params += "__ccg=EXCELLENT&";
  params += "__rev=1016424566&";
  params += "__s=1kvx9u%3Abmomz7%3Amtf05m&";
  params += "__hsi=7413658348684253229&";
  params += "__comet_req=15&";
  params += `fb_dtsg=${fbDtsg}&`;
  params += "jazoest=25670&";
  params += "lsd=IKveao0CHbxsEQO96owePJ&";
  params += "__spin_r=1016424566&";
  params += "__spin_b=trunk&";
  params += "__spin_t=1726126845&";
  params += "fb_api_caller_class=RelayModern&";
  params += "fb_api_req_friendly_name=ProfileCometAboutAppSectionQuery&";
  params += `variables=${encodeURIComponent(JSON.stringify(variables))}&`;
  params += 'server_timestamps=true&';
  params += 'doc_id=8244755638944021';
  ...
  xhr.setRequestHeader("x-fb-lsd", "IKveao0CHbxsEQO96owePJ");
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
        ...
        try {
            ...
            let workData = response.data.user.about_app_sections.nodes.flatMap(section => 
                section.activeCollections.nodes.flatMap(collection => 
                    collection.style_renderer.profile_field_sections.flatMap(fieldSection => 
                        fieldSection.profile_fields.nodes.map(field => {
                            let workTitle = field?.title?.text || '';
                            let workTenure = field?.list_item_groups[0]?.list_items[0]?.text?.text || '';
                            let workDescription = field.list_item_groups[1]?.list_items[0]?.text?.text || '';
                            return {
                                workTitle,
                                workTenure,
                                workDescription
...
var targetUser = prompt("Enter UserID", "100002907575642");
